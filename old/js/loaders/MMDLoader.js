...
THREE.MMDLoader.prototype.pourVmdIntoCamera = function (camera, vmd, name) {
  ...
  var initAnimation = function () {
    ...
    tracks.push(
      helper.generateTrackFromAnimationKeys(
        pkeys,
  ...
  var parseFrames = function () {
    var parseFrame = function () {
      ...
      for (var i = 0; i < p.elementCount; i++) {
        ...
        e.index =
          e.target === 0
            ? dv.getIndex(pmx.metadata.boneIndexSize)
            : dv.getIndex(pmx.metadata.morphIndexSize)
        p.elements.push(e)
      }
...
THREE.MMDLoader.prototype.parseVmd = function (buffer) {
  ...
  var parseHeader = function () {
    ...
    if (metadata.magic !== 'Vocaloid Motion Data 0002') {
      throw (
        'VMD file magic is not Vocaloid Motion Data 0002, but ' + metadata.magic
      )
    }
...
THREE.MMDLoader.prototype.parseVpd = function (text) {
  ...
  function parseBones() {
    ...
    var boneVectorPattern =
      /^\s*(-?[0-9]+\.[0-9]+)\s*,\s*(-?[0-9]+\.[0-9]+)\s*,\s*(-?[0-9]+\.[0-9]+)\s*;/
    var boneQuaternionPattern =
...
THREE.MMDLoader.prototype.createMesh = function (
  ...
  var initMaterials = function () {
    ...
    function loadTexture(filePath, params) {
      ...
      var directoryPath =
        params.defaultTexturePath === true
          ? scope.defaultTexturePath
          : texturePath
    ...
    for (var i = 0; i < model.metadata.materialCount; i++) {
      ...
      params.name = m.name
      params.color = color
        .fromArray([m.diffuse[0], m.diffuse[1], m.diffuse[2]])
        .getHex()
      params.opacity = m.diffuse[3]
      params.specular = color
        .fromArray([m.specular[0], m.specular[1], m.specular[2]])
        .getHex()
      ...
      if (params.map === undefined /* && params.envMap === undefined */) {
        params.emissive = color
          .fromArray([m.emissive[0], m.emissive[1], m.emissive[2]])
          .getHex()
      }
      ...
      if (m.map !== null) {
        ...
        function checkTextureTransparency(m) {
          m.map.readyCallbacks.push(function (t) {
            ...
            var uvs = geometry.faceVertexUvs[0].slice(
              m.faceOffset,
              m.faceOffset + m.faceNum
            )
      ...
      if (model.metadata.format === 'pmd') {
        ...
        if (p2.toonIndex === -1) {
          ...
          var uuid = loadTexture(n, {
            defaultTexturePath: isDefaultToonTexture(n),
          })
        ...
        m.uniforms.outlineColor.value = new THREE.Color(
          p2.edgeColor[0],
          p2.edgeColor[1],
          p2.edgeColor[2]
        )
...
THREE.MMDLoader.DataCreationHelper.prototype = {
  ...
  pushBoneAnimationKey: function (keys, time, bone, pos, rot) {
    keys.push({
      time: time,
      pos: [bone.pos[0] + pos[0], bone.pos[1] + pos[1], bone.pos[2] + pos[2]],
